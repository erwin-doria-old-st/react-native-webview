...
const resolveAssetSource = (source: ImageSourcePropType) =>
    ...
    (
        ...
        useImperativeHandle(
            ...
            () => ({
                goForward: () =>
                    webViewRef.current &&
                    Commands.goForward(webViewRef.current),
                goBack: () =>
                ...
                stopLoading: () =>
                    webViewRef.current &&
                    Commands.stopLoading(webViewRef.current),
                ...
                requestFocus: () =>
                    webViewRef.current &&
                    Commands.requestFocus(webViewRef.current),
                clearFormData: () =>
                    webViewRef.current &&
                    Commands.clearFormData(webViewRef.current),
                clearCache: (includeDiskFiles: boolean) =>
                    webViewRef.current &&
                    Commands.clearCache(webViewRef.current, includeDiskFiles),
                clearHistory: () =>
                    webViewRef.current &&
                    Commands.clearHistory(webViewRef.current),
            }),
        ...
        if (typeof source !== 'number' && source && 'method' in source) {
            if (source.method === 'POST' && source.headers) {
                ...
                console.warn(
                    'WebView: `source.body` is not supported when using GET.',
                );
            }
        }
        ...
        // oxlint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
        const sourceResolved = resolveAssetSource(
...
const isFileUploadSupported = async () =>
    RNCWebViewModule.isFileUploadSupported();
